...
test('account access has a premium dark mobile-safe visual layer', () => {
  ...
  assert.match(appHtml, /#welcome-modal \.welcome-card\s*\{[\s\S]*?border-radius:24px !important;|#welcome-modal \.welcome-card\s*\{[\s\S]*?border-radius: 24px !important;/);
  assert.match(appHtml, /#welcome-modal \.own-input\s*\{[\s\S]*?background:#0d0f13 !important;|#welcome-modal \.own-input\s*\{[\s\S]*?background: #0d0f13 !important;/);
  assert.match(appHtml, /@media \(max-width: 520px\) \{[\s\S]*?#welcome-modal \.welcome-card\s*\{[\s\S]*?border-radius:24px 24px 0 0 !important;|@media \(max-width: 520px\) \{[\s\S]*?#welcome-modal \.welcome-card\s*\{[\s\S]*?border-radius: 24px 24px 0 0 !important;/);
});
