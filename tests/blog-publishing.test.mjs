...
test('every sitemap article resolves to a real complete HTML document with searchable content',()=>{
  assert.equal(urls.length,74);
  for(const url of urls){
    const route=config.rewrites.find(r=>r.source===url.pathname);
    assert.equal(route?.destination,url.pathname+'.html',url.href);
    ...
    assert.match(page,/background:#0a0a0a;color:#fff/);
    assert.match(page,/<a class="button" href="https:\/\/trystellarai.com\/app">Create your first script free/);
    const body=page.match(/<div class="article-content">([\s\S]*?)<\/div>\s*<section class="related"/)[1];
    assert.ok(body.replace(/<[^>]+>/g,' ').trim().split(/\s+/).length>=800,url.href);
...
test('Roblox-only guides exclude FiveM installation commands',()=>{
  for(const url of urls.filter(u=>u.pathname.startsWith('/blog/roblox-'))){
    const page=readFileSync(new URL(url.pathname.slice(1)+'.html',root),'utf8');
