...
test('Task 188 keeps Roblox games and groups visible with approved destinations', () => {
  assert.match(landingHtml, /<div class="roblox-directory" aria-label="Stellar Roblox games and groups">[\s\S]*Explore Stellar projects[\s\S]*Stellar Strike[\s\S]*Stellar Simulator[\s\S]*StellarHQ[\s\S]*zitos gang[\s\S]*<\/div>/);
  assert.match(landingHtml, /src="\/lib\/assets\/stellar-strike-thumb\.png"[^>]*alt="Stellar Strike lightning star thumbnail"/);
  assert.match(landingHtml, /src="\/lib\/assets\/stellar-simulator-thumb\.png"[^>]*alt="Stellar Simulator orbital planet thumbnail"/);
