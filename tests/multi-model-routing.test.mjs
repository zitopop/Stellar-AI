...
test('uncertain coding requests use a bounded recovery contract instead of giving up', () => {
  ...
  assert.match(UNCERTAINTY_RECOVERY_GUIDANCE, /one bounded second pass/);
  assert.match(UNCERTAINTY_RECOVERY_GUIDANCE, /never loop indefinitely/);
  assert.match(buildSystemPrompt('', 'fivem', 'fivem_resource', 'qbcore', 'implementer'), /UNCERTAINTY RECOVERY/);
});
...
test('Task 225 keeps every permitted public tier available to Lite', () => {
  const permittedAliases = {
    spark: ['spark', 'fabie', 'haiku'],
    star: ['star', 'smart', 'sonnet'],
    comet: ['comet', 'opus'],
  ...
  for (const [tier, aliases] of Object.entries(permittedAliases)) {
    for (const alias of aliases) {
      assert.equal(resolveModelTier(alias, 'lite'), tier, `${alias} must remain available to Lite`);
    }
  }
});
...
test('Task 226 keeps every authorised tier available to Pro and owner', () => {
  ...
  for (const [tier, aliases] of Object.entries(authorisedAliases)) {
    for (const alias of aliases) {
      assert.equal(resolveModelTier(alias, 'pro'), tier, `${alias} must remain available to Pro`);
      assert.equal(resolveModelTier(alias, 'owner'), tier, `${alias} must remain available to owner`);
    }
  }
});
...
test('Task 228 keeps normalized direct alias routing intact for Pro and owner', () => {
  const anthropicAliases = {
    spark: ['spark', 'fabie', 'haiku', 'claude-haiku-4-5-20251001'],
    star: ['star', 'smart', 'sonnet', 'claude-sonnet-5'],
    comet: ['comet', 'opus', 'claude-opus-5'],
  ...
  for (const [tier, aliases] of Object.entries(anthropicAliases)) {
    for (const alias of aliases) {
      ...
      for (const plan of ['pro', 'owner']) {
        const route = resolveRoute(normalizedAlias, '', plan);
        assert.equal(route.provider, 'anthropic', `${alias} must use Anthropic for ${plan}`);
        assert.equal(route.tier, tier, `${alias} must route to ${tier} for ${plan}`);
      }
    }
