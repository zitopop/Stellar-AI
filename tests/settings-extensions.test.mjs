...
const currency = fs.readFileSync(new URL('../assets/js/currency.js', import.meta.url), 'utf8');
const extensionJs = fs.readFileSync(new URL('../assets/js/stellar-settings-extensions.js', import.meta.url), 'utf8');
const extensionCss = fs.readFileSync(new URL('../assets/css/stellar-settings-extensions.css', import.meta.url), 'utf8');
...
test('currency loader includes the isolated Settings extension assets', () => {
  ...
  assert.doesNotMatch(currency, /script\.src = '\/assets\/js\/stellar-orbit\.js/);
});
