...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const qty   = parseInt(session.metadata?.qty, 10) || 0;
    ...
    if (plan === 'topup' && qty > 0) {
      ...
      const bonusFor = (p) => p >= 5000 ? Math.round(p * 0.25) : p >= 2500 ? Math.round(p * 0.20) : p >= 1000 ? Math.round(p * 0.15) : p >= 500 ? Math.round(p * 0.10) : 0;
