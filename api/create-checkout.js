...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── LED Theme — one-time £20 (inline price — no pre-made Stripe price/env var needed) ──
    if (mode === 'led') {
      const session = await stripePost('/checkout/sessions', {
        ...
        'line_items[0][price_data][product_data][name]': 'Stellar AI — LED Theme (lifetime)',
        'line_items[0][price_data][product_data][description]': 'Exclusive neon LED theme — yours forever, one-time purchase',
