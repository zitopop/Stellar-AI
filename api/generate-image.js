...
export default async function handler(req, res) {
  ...
  try {
    ...
    const imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(safePrompt)}?width=${isHD ? 1024 : 512}&height=${isHD ? 1024 : 512}&nologo=true${pollKey ? `&apikey=${pollKey}` : ''}`;

    // Fetch and proxy the image so we don't expose the API key or URL
    ...
    res.status(500).json({ error: 'Could not generate image. Try again.' });
  }
}
