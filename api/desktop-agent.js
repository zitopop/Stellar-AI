...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='poll'){
      ...
      await redis('set',`stellar:desktop:task:${task.id}`,JSON.stringify({...task,status:'running',startedAt:Date.now()}));await redis('expire',`stellar:desktop:task:${task.id}`,TASK_TTL_SECONDS);
    ...
    if(action==='report'){
      ...
      const result={taskId,ok:req.body?.ok===true,output:String(req.body?.output||'').slice(0,120000),error:String(req.body?.error||'').slice(0,4000),finishedAt:Date.now()};
    ...
    if(action==='createPair'){
      ...
      const code=crypto.randomBytes(6).toString('hex').toUpperCase(),pair={email:session.email,expiresAt:Date.now()+PAIR_TTL_SECONDS*1000};
    ...
    if(action==='emergencyStop'){
      ...
      const id=String(await redis('get',`stellar:desktop:owner:${session.email}`)||'');if(id)await redis('del',`stellar:desktop:queue:${id}`);
    ...
    if(action==='status'){
      ...
      if(!deviceId)return res.status(200).json({ok:true,paired:false,emergencyStopped:stopped,policy});
      ...
      return res.status(200).json({ok:true,paired:true,deviceId,hostname:d.hostname,platform:d.platform,lastSeen:d.lastSeen,online:!stopped&&Date.now()-Number(d.lastSeen||0)<90000,lastResult,emergencyStopped:stopped,policy});
    }
    ...
    if(action==='enqueue'){
      ...
      await redis('set',`stellar:desktop:task:${task.id}`,JSON.stringify({...task,status:'queued'}));await redis('expire',`stellar:desktop:task:${task.id}`,TASK_TTL_SECONDS);
    ...
    if(action==='taskStatus'){
      ...
      let task;try{task=JSON.parse(raw)}catch{return res.status(500).json({error:'Task state is invalid.'})}return res.status(200).json({ok:true,task});
    }
    if(action==='disconnect'){
      await redis('del',`stellar:desktop:owner:${session.email}`);await redis('del',`stellar:desktop:device:${deviceId}`);await redis('del',`stellar:desktop:queue:${deviceId}`);
