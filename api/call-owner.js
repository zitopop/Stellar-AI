...
export default async function handler(req, res) {
  ...
  try {
    const bridge = await fetch(BRIDGE_URL, {
      ...
      body: JSON.stringify({ purpose: String(req.body?.purpose || 'Owner requested a call from Jarvis in Stellar AI.').slice(0, 300) }),
    });
    const data = await bridge.json().catch(() => ({}));
    return res.status(bridge.status).json(data);
  } catch (error) {
    ...
    if (!response.ok) {
      const message = data?.error || data?.message || 'The phone service could not start the call.';
    ...
    console.error('Owner call bridge error', error?.message || error);
    return res.status(502).json({ error: 'The phone service is temporarily unavailable.' });
  }
}
