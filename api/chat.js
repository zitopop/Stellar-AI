...
export default async function handler(req, res) {
  try {
    ...
    // Agentic loop: let Claude use tools (web search), then answer
    let finalText = '';
    ...
    for (let turn = 0; turn < 6; turn++) {
      const apiRes = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model, max_tokens, system, messages: convo, tools: TOOLS, stream: false })
      });
      ...
      if (!finalText && data.stop_reason === 'max_tokens') {
        finalText = 'That answer was very long and got cut off. Try asking for it in smaller parts — e.g. one script at a time.';
      }
