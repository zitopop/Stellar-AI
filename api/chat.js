...
const ROLE_OUTPUT_CONTRACTS = {
  ...
  money: 'ROLE OUTPUT CONTRACT: Start with the safest realistic offer, target buyer, price range, proof needed, and first asset to build. Then provide concrete deliverables such as landing-page copy, audit checklist, outreach draft, delivery SOP, pricing/options, validation steps, and what not to automate. Never promise guaranteed income or advise spam, deception, bots, fake reviews, credential collection, or platform-rule evasion.',
...
- Include a short numbered Studio setup checklist with exact Explorer destinations and object types, then a test matrix covering first join, reconnect, invalid remote input, duplicate request, player leaving during a save, failed DataStore call, and the main happy path. Never claim the game was run or published.
...
- Do not leave TODOs, placeholder functions or omitted critical logic in code presented as complete. After the code, include a WHAT TO DO NEXT checklist with exact destinations, configuration changes, commands/restarts or Studio actions, and a CHECK IT WORKS checklist with observable expected results.
...
const WORKFLOW_GUIDANCE = {
  roblox_build_pack: `WORKFLOW MODE: ROBLOX BUILD PACK\nStart with a compact implementation plan and exact Studio file tree before code. For every remote, list server validation for types, ranges, ownership, cooldowns, permissions, and duplicate requests. For persistence, state the key shape, pcall behavior, and SetAsync versus UpdateAsync choice. End with numbered Studio instructions naming the exact Explorer destination and object type for every file or remote, any required properties or API Services setting, the Play/Test action to run, and the expected result. Include a test matrix and manual art/animation/plugin steps if required. Never claim the place was run or published.`,
  fivem_resource: `WORKFLOW MODE: FIVEM RESOURCE\nStart with a resource manifest and dependency/framework assumptions. Name each required client, server, shared, config, and SQL file, keep rewards, money, permissions, and validation server-side, and include protected network-event checks. End with numbered install steps that name the resource folder, any SQL/import step, the exact server.cfg ensure line when applicable, restart order, one happy-path test, and one failure or permission test. Never claim the resource was installed or run.`,
  ...
  money_builder: `WORKFLOW MODE: MONEY BUILDER / SIDE HUSTLE ASSET\nHelp the user create legitimate, sellable assets and service workflows. Start with one clear offer, buyer, expected delivery time, low-budget tool stack, price range, proof needed, and risk notes. Prefer fast services that Stellar can help deliver: website mini-audits, AI receptionist setup packs, Shopify cleanup, SEO/blog packs, Discord/FiveM/Roblox setup help, and small automation/document packs. Produce concrete assets such as landing-page copy, audit checklist, outreach draft, delivery SOP, pricing/options, fulfilment steps, and follow-up messages. Keep claims honest: do not promise guaranteed income, do not recommend spam, bots, fake reviews, deception, credential collection, regulated/high-risk schemes, or platform-rule evasion. End with the first asset to create and the first safe customer validation step.`,
