...
function resolveModel(requestedModel, plan) {
  ...
  const resolved = MODEL_MAP[requestedModel?.toLowerCase()] || requestedModel;
  // Check if plan allows this model
  if (limits.models.includes(resolved)) return resolved;
  // Default to best allowed model for plan
  return limits.models[limits.models.length - 1];
}
