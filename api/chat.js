...
export default async function handler(req, res) {
  ...
  // If KV has a confirmed record use it — otherwise trust what the client says
  // (protects against server-side abuse while not breaking legit users with no KV record yet)
  const clientPlan = _client_plan || 'free';
  const plan = kvPlan !== null ? kvPlan : clientPlan;
  ...
  if (isOpus) {
    ...
    // Only block if KV CONFIRMED the user is free — don't block if plan is unknown
