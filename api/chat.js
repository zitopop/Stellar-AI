...
export default async function handler(req, res) {
  try {
    ...
    // ════ FAST PATH: no tools needed → stream Claude straight to the user (no timeout on big code) ════
    if (wantsStream && !activeTools.length) {
      ...
      if (!upstream.ok || !upstream.body) {
        ...
        const msg = (errData && errData.error && errData.error.message) ? errData.error.message : ('Claude API error ' + upstream.status);
        return res.status(200).json({ error: msg });
      }
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache, no-transform');
      res.setHeader('Connection', 'keep-alive');
      // Pipe Anthropic's SSE straight through — the client already understands content_block_delta
      ...
      while (true) {
        ...
        if (chunk.includes('text_delta')) gotText = true;
        res.write(chunk);
      }
      if (!gotText) {
        res.write('data: ' + JSON.stringify({ type: 'content_block_delta', delta: { type: 'text_delta', text: 'Hmm, I could not generate that — please try again.' } }) + '\n\n');
      }
      res.write('data: ' + JSON.stringify({ type: 'message_stop' }) + '\n\n');
      return res.end();
    }

    // ════ TOOL PATH: needs web search → run the agent loop, then return the text ════
    let finalText = '';
    ...
    for (let turn = 0; turn < 4; turn++) {
      const apiRes = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
        body: JSON.stringify({ model, max_tokens, system, messages: convo, tools: activeTools, stream: false })
      });
      ...
      finalText = content.filter(b => b && b.type === 'text').map(b => b.text).join('');
      if (!finalText) finalText = 'Hmm, I could not generate that. Please try rephrasing.';
      break;
    }
    ...
    for (let i = 0; i < finalText.length; i += size) {
      res.write('data: ' + JSON.stringify({ type: 'content_block_delta', delta: { type: 'text_delta', text: finalText.slice(i, i + size) } }) + '\n\n');
    }
    res.write('data: ' + JSON.stringify({ type: 'message_stop' }) + '\n\n');
    return res.end();
