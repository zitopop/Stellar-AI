...
// Server-side allowlist — client cannot send arbitrary model IDs
...
export default async function handler(req, res) {
  ...
  const { model: rawModel, messages, system, max_tokens, _client_plan } = req.body || {};

  if (!rawModel || !Array.isArray(messages)) {
    return res.status(400).json({ error: 'model and messages required' });
  }

  // Enforce allowlist — reject unknown models
  ...
  try {
    const upstream = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model, max_tokens: safeMax, system: system || '', messages, stream: true }),
    });
