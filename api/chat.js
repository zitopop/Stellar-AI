...
export default async function handler(req, res) {
  ...
  const plan     = await getPlanFromKV(_email);
  const isPaid   = plan !== 'free';
  const rl       = await checkRateLimit(ip, isPaid);
  ...
  if (isOpus) {
    // Power model needs Pro+, Ultra needs Max+
    const needsMax = modelKey === 'ultra';
    if (needsMax && !['max', 'ultimate'].includes(plan)) {
      return res.status(403).json({ error: 'Ultra model requires Max plan. Upgrade to unlock.' });
    }
    if (!needsMax && !['pro', 'max', 'ultimate'].includes(plan)) {
      return res.status(403).json({ error: 'Power model requires Pro plan. Upgrade to unlock.' });
    }
  }
