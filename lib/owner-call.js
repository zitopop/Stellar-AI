...
function hasCallStorage() {
  return Boolean(process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN);
}

async function saveCallContext(purpose, fallback = null) {
  const url = process.env.KV_REST_API_URL;
  const token = process.env.KV_REST_API_TOKEN;
  if (!url || !token) throw new Error('Jarvis call storage is unavailable.');
  const id = randomUUID();
  const value = JSON.stringify({ purpose, fallback, createdAt: Date.now(), mode: 'outbound-owner' });
  const command = ['SET', `stellar:jarvis:call-context:${id}`, value, 'EX', 3600];
...
async function startTwilioOwnerCall(purpose, fallback = null) {
  const config = twilioConfig();
  if (!hasTwilioConfig(config)) throw new Error('Twilio owner calling is not configured.');
  const contextId = await saveCallContext(purpose, fallback);
  const voiceUrl = `${PUBLIC_URL}/api/broadcast?jarvisVoice=1&context=${encodeURIComponent(contextId)}`;
  const statusUrl = `${PUBLIC_URL}/api/broadcast?jarvisCallStatus=1&context=${encodeURIComponent(contextId)}`;
...
export function getOwnerCallConfiguration() {
  const config = twilioConfig();
  return {
    twilioConfigured: hasTwilioConfig(config) && hasCallStorage(),
    twilioCredentialsConfigured: hasTwilioConfig(config),
    callStorageConfigured: hasCallStorage(),
    ownerNumberConfigured: E164.test(config.to),
    bridgeConfigured: Boolean(process.env.CALL_BRIDGE_TOKEN),
  };
}

export async function startOwnerCall({ purpose, authorization = '', bridgeToken = '', fallback = null }) {
  const safePurpose = String(purpose || 'Jarvis needs the owner.').trim().slice(0, 300);
  const failures = [];

  if (hasTwilioConfig() && hasCallStorage()) {
    try {
      return await startTwilioOwnerCall(safePurpose, fallback);
    } catch (error) {
      failures.push(error);
...
export async function readOwnerCallHealth({ authorization = '', bridgeToken = '' }) {
  ...
  if (!bridgeToken && !authorization) {
    return { ok: true, ready: false, provider: 'none', twilio: false, retell: false };
  }
  const headers = { 'Content-Type': 'application/json' };
  if (authorization) headers.Authorization = authorization;
  if (bridgeToken) headers['x-call-bridge-token'] = bridgeToken;
  try {
    ...
    return { ok: false, ready: false, provider: 'retell', twilio: false, retell: false, error: 'Phone bridge unavailable.' };
  }
}
