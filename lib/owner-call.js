...
export async function readOwnerCallHealth({ authorization = '', bridgeToken = '' }) {
  ...
  if (configuration.twilioConfigured) {
    const storageReachable = configuration.callContextStorageConfigured
      ...
      : false;
    const missing = [];
    if (!configuration.ownerNumberConfigured) missing.push('OWNER_PHONE must be a valid +country number');
    if (!configuration.callContextStorageConfigured) missing.push('KV_REST_API_URL and KV_REST_API_TOKEN');
    else if (!storageReachable) missing.push('call context storage is unreachable');
  ...
  if (configuration.telnyxConfigured) {
    ...
    const configured = configuration.ownerNumberConfigured && storageReachable;
    return buildCallReadiness({
      provider: 'telnyx',
      configured,
      ready: configured,
      ...
      extra: {
        ...
        configuredAgent: true,
        configuredNumber: true,
        error: configured ? undefined : 'Telnyx owner calling is missing storage or owner phone readiness.',
      },
    });
  }
