...
function twilioMissing(config = twilioConfig()) {
  ...
  else if (!E164.test(config.from)) missing.push('TWILIO_FROM_NUMBER must look like +15551234567');
  if (!config.to) missing.push('OWNER_PHONE');
  else if (!E164.test(config.to)) missing.push('OWNER_PHONE must look like +15551234567');
  return missing;
}
