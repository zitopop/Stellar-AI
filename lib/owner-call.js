...
async function saveCallContext(purpose) {
  const url = process.env.KV_REST_API_URL;
  const token = process.env.KV_REST_API_TOKEN;
  if (!url || !token) throw new Error('Jarvis call storage is unavailable.');
  const id = randomUUID();
  const value = JSON.stringify({ purpose, createdAt: Date.now(), mode: 'outbound-owner' });
...
async function startTwilioOwnerCall(purpose) {
  const config = twilioConfig();
  if (!hasTwilioConfig(config)) throw new Error('Twilio owner calling is not configured.');
  const contextId = await saveCallContext(purpose);
...
export async function startOwnerCall({ purpose, authorization = '', bridgeToken = '' }) {
  const safePurpose = String(purpose || 'Jarvis needs the owner.').trim().slice(0, 300);
  const failures = [];

  if (hasTwilioConfig()) {
    try {
      return await startTwilioOwnerCall(safePurpose);
    } catch (error) {
      failures.push(error);
