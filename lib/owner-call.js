...
export function getOwnerCallConfiguration() {
  const config = twilioConfig();
  return {
    twilioConfigured: hasTwilioConfig(config) && hasCallContextStorage(),
    twilioCredentialsConfigured: hasTwilioConfig(config),
    callStorageConfigured: hasCallContextStorage(),
...
export async function startOwnerCall({ purpose, authorization = '', bridgeToken = '', fallback = null }) {
  ...
  if (hasTwilioConfig() && hasCallContextStorage()) {
    try {
...
export async function readOwnerCallHealth({ authorization = '', bridgeToken = '' }) {
  ...
  if (configuration.twilioConfigured) {
    const configured = configuration.twilioConfigured
      ...
      && configuration.callContextStorageConfigured;
    return {
      ok: true,
