...
const ALLOWED=new Set(['read_file','list_directory','search_files','git_status','git_diff','apply_patch','write_file','mkdir','run_command','keyboard_shortcut','type_text','open_url']);
...
- keyboard_shortcut {shortcut} — safe allowlist only: CTRL+TAB, CTRL+SHIFT+TAB, ALT+TAB, CTRL+L, CTRL+W, CTRL+R\n- type_text {text} — type approved text into the currently focused field; NEVER presses Enter or sends the message\n- open_url {url}
...
- Prefer apply_patch over write_file for existing files because it makes a precise evidence-grounded replacement and preserves unrelated content. Use write_file for new files or when a full replacement is genuinely required.\n- Treat apply_patch, write_file, mkdir, run_command, keyboard_shortcut, type_text, and open_url as approval-required actions.\n- For messaging tasks, type_text may fill a compose box, but do not send, submit, press Enter, click Send, or confirm a consequential action unless the user separately and explicitly asks for that send action through an authorized communication tool. Do not mark or imply approval yourself.
