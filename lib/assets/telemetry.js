(() => {
  ...
  function supportFallback(event) {
    ...
    if (!/(support|account help|billing|refund|copy email|contact)/.test(text)) return;
  ...
  function preventPinnedDelete(event) {
    ...
    if (!row) return;
    const pinned = row.matches?.('[data-pinned="true"') || /pinned|📌|pin-mark/.test(row.textContent || '') || row.querySelector?.('.pin-mark,[data-pin-state="pinned"]');
    if (!pinned) return;
    event.preventDefault();
