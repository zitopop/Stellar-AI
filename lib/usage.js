import { OVERAGE_REQUEST_COST_PENCE, getPlanDefinition, getWeeklyRequestLimit, normalisePlan } from './pricing.js';
...
function weeklyCounterKey(identity, now) {
  return `stellar:usage:week:${weekWindow(now).startAt}:${identityKey(identity)}`;
}
...
export async function consumeUsage({ url, token, identity, plan, walletKey = '', allowCredit = false, creditCostPence = OVERAGE_REQUEST_COST_PENCE, now = Date.now() }) {
  ...
  const canonicalPlan = normalisePlan(plan) || 'free';
  const definition = getPlanDefinition(canonicalPlan);
  const weeklyLimit = getWeeklyRequestLimit(canonicalPlan);
  let weekly = null;

  if (weeklyLimit) {
    const week = weekWindow(now);
    const weeklyUsed = await kvIncrement(url, token, weeklyCounterKey(identity, now), week.ttlSeconds);
    weekly = {
      percent: Math.min(100, Math.round((weeklyUsed / weeklyLimit) * 100)),
      resetAt: new Date(week.resetAt).toISOString(),
      low: weeklyUsed / weeklyLimit >= 0.8,
      exhausted: weeklyUsed > weeklyLimit,
    };
    if (weeklyUsed > weeklyLimit && !allowCredit) {
      return { ...usageSnapshotFromRecord(canonicalPlan, 0, now), allowed: false, reason: 'weekly', weekly, chargedCreditPence: 0, walletPence: null, creditCostPence };
    }
  }
  ...
  if (allowCredit && walletKey) {
    ...
    return { ...usageSnapshotFromRecord(canonicalPlan, count, now), allowed, weekly, chargedCreditPence, walletPence, creditCostPence };
  }
  ...
  return { ...snapshot, allowed: snapshot.used <= snapshot.limit, weekly, chargedCreditPence: 0, walletPence: null, creditCostPence };
}
...
export { CREDIT_USAGE_LUA, REFUND_CREDIT_LUA, hourWindow, weekWindow, identityKey };
