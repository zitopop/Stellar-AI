...
const WRITES=new Set(['ensure_folder','ensure_remote_event','ensure_remote_function','upsert_script','create_map_pack']);
...
function validate(plan){
  ...
  const actions=(Array.isArray(plan?.actions)?plan.actions:[]).slice(0,16).map((action,index)=>{
    ...
    if(type==='create_map_pack'){
      args.rootPath=String(args.rootPath||'Workspace/StellarGeneratedMap').slice(0,900);
      args.theme=String(args.theme||'Stellar Roblox map').slice(0,120);
      args.clearExisting=args.clearExisting===true;
      args.parts=(Array.isArray(args.parts)?args.parts:[]).slice(0,120);
      args.spawnPoints=(Array.isArray(args.spawnPoints)?args.spawnPoints:[]).slice(0,16);
      args.lighting=args.lighting&&typeof args.lighting==='object'&&!Array.isArray(args.lighting)?args.lighting:{};
      if(!args.parts.length&&!args.spawnPoints.length)throw new Error('Planner returned an empty map pack.');
    }
...
- create_map_pack {rootPath:"Workspace/StellarGeneratedMap",theme,clearExisting,lighting:{clockTime,brightness,fogEnd},parts:[{name,position:{x,y,z},size:{x,y,z},color:{r,g,b},material,shape,anchored,canCollide,transparency}],spawnPoints:[{name,position:{x,y,z},size:{x,y,z},color:{r,g,b},material}]
...
- For requests to build a real map, obby, simulator lobby, island, arena, city, tycoon starter, roleplay town, or game world, use create_map_pack in the implement phase so the Studio plugin creates visible Workspace geometry, spawn points, lighting and materials immediately. Do not only write instructions or scripts for maps.
- Keep generated map packs under Workspace/StellarGeneratedMap unless observations show a better dedicated folder. If replacing a previous generated pack, set clearExisting true only for that generated map folder, never for Workspace itself.
