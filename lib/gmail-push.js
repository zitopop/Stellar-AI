...
async function callOwnerForEmail(message) {
  ...
  const purpose = `A new email arrived from ${sender}. Subject: ${subject}. Route: ${route.team} because it looks like ${route.reason}. I am calling because you asked me to call whenever a new inbox email arrives.`.slice(0, 420);
  try {
    ...
    return { called: true, provider: result?.provider || null, call_id: result?.call_id || null };
  } catch (error) {
