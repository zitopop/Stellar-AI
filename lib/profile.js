...
/**
 * Awards 100 promotional Stellar Credits exactly once per referred address. A claim is
...
export async function applyReferralReward(url, token, referredEmail, rawCode) {
  ...
  const creditedReferrer = {
    ...referrerUser,
    achievements: { ...(referrerUser.achievements || {}), 'referral-builder': now },
...
export function achievementDefinitions() {
  return [
    ...
    { id: 'hundred-scripts', label: 'Elite Builder', description: 'Generate 100 scripts.' },
    { id: 'return-builder', label: 'Return Builder', description: 'Come back for a second build session.' },
    { id: 'referral-builder', label: 'Referral Builder', description: 'Bring a builder to Stellar.' },
...
export async function recordAcceptedRequest(url, token, email, now = Date.now()) {
  ...
  if (startsNewSession && Number(prior.sessionCount) >= 1 && !achievements['return-builder']) achievements['return-builder'] = now;
  await kvSet(url, token, userKey, { ...user, usage, achievements, updatedAt: now });
  return usage;
}
...
export async function recordScriptGenerated(url, token, email) {
  ...
  if (count >= 100 && !achievements['hundred-scripts']) achievements['hundred-scripts'] = now;
