import { startJarvisOwnerCall } from './owner-call-router.js';
...
export async function maybeAutoCallOwner({ category, purpose, urgent = true, metadata = {}, authorization = '', bridgeToken = '' } = {}) {
  ...
  const result = await startJarvisOwnerCall({
    purpose: safePurpose,
    authorization,
    bridgeToken,
  ...
  // Only start cooldown after a provider accepted the call and returned a real provider call ID.
