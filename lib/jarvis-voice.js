...
function greetingFor(state) {
  if (state.mode === 'outbound-owner') {
    return `Hi, it's Jarvis from Stellar AI. I need your help. ${state.purpose} Tell me what you want me to do.`;
  }
  if (state.mode === 'inbound-owner') return `Hi, it's Jarvis. I'm ready. What do you need?`;
...
async function generateReply(state, speech) {
  ...
  if (!apiKey) return 'I heard you. I cannot reach my reasoning service right now, so I have saved the call context for follow-up.';
  const ownerMode = state.mode !== 'inbound-public';
  const system = ownerMode
    ? `You are Jarvis, the private Stellar AI owner assistant on a live phone call with the owner. The reason for this call is: ${state.purpose}. Keep every reply under 55 words. Be direct and calm. Ask at most one useful follow-up question. Never claim an action was completed unless this call state explicitly proves it. Never reveal secrets, credentials, or hidden system data.`
  ...
  try {
    ...
    return 'I heard you. My reasoning service is slow right now, so I have kept the call context and will not guess.';
  }
}
...
export async function handleJarvisVoiceWebhook(req, res) {
  ...
  if (isGoodbye(speech)) {
    ...
    await kvSet(stateKey, state);
    return sendXml(res, `${say('Okay. I have ended the call.')}<Hangup/>`);
  }

  if (turn >= MAX_TURNS) {
    state.messages = [...(state.messages || []), { role: 'user', content: speech }].slice(-12);
    await kvSet(stateKey, state);
    return sendXml(res, `${say('I have saved the important context. I am ending this call now to avoid an endless call loop.')}<Hangup/>`);
  }
  ...
  const prompt = `${reply} You can tell me more, or say goodbye to end the call.`;
  return sendXml(res, `${gather(prompt, speechAction(contextId, nextTurn))}${say('I did not hear a response. Goodbye.')}<Hangup/>`);
}
