...
export async function handleJarvisVoiceWebhook(req, res) {
  ...
  if (Number.isFinite(confidence) && confidence > 0 && confidence < 0.22) {
    ...
    return sendXml(res, `${gather(repeatPrompt, speechAction(contextId, Math.min(turn + 1, MAX_TURNS)))}${say(ownerMode ? 'I still could not hear you clearly, sir. Goodbye.' : 'I still could not hear you clearly. Goodbye.')}<Hangup/>`);
  }
  ...
  return sendXml(res, `${gather(reply, speechAction(contextId, nextTurn))}${say(ownerMode ? 'I did not hear a response, sir. Goodbye.' : 'I did not hear a response. Goodbye.')}<Hangup/>`);
}
