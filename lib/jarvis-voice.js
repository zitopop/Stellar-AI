...
function greetingFor(state) {
  ...
  if (state.mode === 'outbound-public') {
    const name = state.contactName ? ` ${state.contactName}` : '';
    return `Hello${name}. I'm Jarvis, an AI assistant calling on behalf of Tobi at Stellar AI. ${state.purpose} Is now a good time to speak?`;
  }
...
async function generateReply(state, speech) {
  ...
  const ownerMode = state.mode === 'outbound-owner' || state.mode === 'inbound-owner';
