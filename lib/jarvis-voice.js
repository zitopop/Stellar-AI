...
function gather(prompt, action) {
  const hints = 'Jarvis,Stellar,Tobi,sir,Shopify,Vercel,GitHub,Retell,Twilio,FiveM,Roblox,Tebex,security,payment,customer,website,server';
  return `<Gather input="speech" method="POST" action="${xmlEscape(action)}" speechTimeout="2" timeout="5" language="${LANGUAGE}" speechModel="experimental_conversations" hints="${xmlEscape(hints)}" profanityFilter="false" actionOnEmptyResult="true">${say(prompt)}</Gather>`;
}
...
export async function handleJarvisVoiceWebhook(req, res) {
  ...
  if (Number.isFinite(confidence) && confidence > 0 && confidence < 0.4 && turn < MAX_TURNS) {
    const retry = ownerMode ? 'Sorry, sir. I did not catch that clearly. Please say it again.' : 'Sorry, I did not catch that clearly. Please say it again.';
    return sendXml(res, `${gather(retry, speechAction(contextId, turn + 1))}${say(ownerMode ? 'I still did not hear you clearly, sir. Goodbye.' : 'I still did not hear you clearly. Goodbye.')}<Hangup/>`);
  }
