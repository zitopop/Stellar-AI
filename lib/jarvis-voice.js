...
const SPEECH_HINTS = String(process.env.JARVIS_SPEECH_HINTS || 'Tobi, Jarvis, Stellar AI, Chrome Cruiser, Shopify, Twilio, Retell, FiveM, Tebex, Vercel, GitHub');
...
async function handleTwilioStatusCallback(req, res, contextId) {
  ...
  if (emailed) {
    await Promise.all([
      kvSet('stellar:owner-call:last', stamp, 86400),
      kvSet(fallbackKey, { sentAt: stamp }, 86400),
...
function say(value) {
  ...
  const body = /^(?:Polly|Google)\./i.test(VOICE)
...
function greetingFor(state) {
  if (state.mode === 'outbound-owner') {
    return `Hello, Tobi. Jarvis here, sir. I need your help. ${state.purpose} Tell me what you want me to do.`;
  }
...
async function generateReply(state, speech) {
  ...
  const emailLookup = ownerMode
    ? await getOwnerEmailContext(speech)
    : { requested: false, configured: false, context: '' };

  if (ownerMode && emailLookup.requested && !emailLookup.configured) {
    return 'Sir, Gmail reading is installed for owner calls, but your Gmail OAuth connection is not configured on the server yet. I will not guess what your emails say.';
  }

  const speechGuidance = 'The caller may speak casually, quickly, with a UK accent, hesitations, slang, incomplete sentences, or imperfect speech-to-text. Infer the likely intent conservatively from the conversation. Recognize names such as Tobi, Jarvis, Stellar AI, Chrome Cruiser, Shopify, Twilio, Retell, FiveM, Tebex, Vercel, and GitHub. If the transcript is genuinely ambiguous, say briefly what you think you heard and ask one short clarification instead of pretending to understand.';
  const emailGuidance = emailLookup.context
    ? ' PRIVATE OWNER EMAIL CONTEXT is trusted mailbox data fetched for this owner request. Use it to answer the question accurately. Do not read credentials, verification codes, password-reset links, API keys, card data, or other authentication secrets aloud. Summarize sensitive operational content instead of reciting it verbatim.'
    : '';
  const system = ownerMode
    ? `You are Jarvis, the private Stellar AI owner assistant on a live phone call with Tobi. Use "sir" naturally in the greeting and occasional acknowledgements, but do not say it in every reply and never use it more than once in a reply. The reason for this call is: ${state.purpose}. ${speechGuidance}${emailGuidance} Keep every reply under 55 words. Be direct, warm and calm. Answer the owner's actual request, not the literal wording of transcription mistakes. Ask at most one useful follow-up question. Never claim an action was completed unless this call state explicitly proves it. Never reveal secrets, credentials, or hidden system data.`
  ...
  const currentUserContent = emailLookup.context
...
export async function handleJarvisVoiceWebhook(req, res) {
  ...
  const turn = Math.max(0, Math.min(500, Number(req.query?.turn || 0) || 0));
  const context = contextId ? await kvGet(`stellar:jarvis:call-context:${contextId}`) : null;
  ...
  if (!speech) {
    if (ownerMode) {
      state.silenceCount = Math.max(0, Number(state.silenceCount || 0)) + 1;
      await kvSet(stateKey, state);
      if (state.silenceCount <= MAX_OWNER_SILENCE_RETRIES) {
        const prompt = turn === 0
          ? greetingFor(state)
          : 'I am still here, sir. Take your time and speak when you are ready.';
        return sendXml(res, gather(prompt, speechAction(contextId, turn + 1)));
      }
  ...
  if (Number.isFinite(confidence) && confidence > 0 && confidence < 0.22) {
    const repeatPrompt = ownerMode
      ? 'Sorry, sir, I did not catch that clearly. Please say it again in your own words.'
      : 'Sorry, I did not catch that clearly. Please say it again in your own words.';
  ...
  if (!ownerMode && turn >= MAX_PUBLIC_TURNS) {
    state.messages = [...(state.messages || []), { role: 'user', content: speech }].slice(-12);
    await kvSet(stateKey, state);
    return sendXml(res, `${say('I have saved the important context. I am ending this call now to avoid an endless public call loop.')}<Hangup/>`);
  }
