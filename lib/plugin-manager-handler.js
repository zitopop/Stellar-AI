...
const LIVE_OAUTH_PLUGIN_IDS=new Set(['github']);
...
export default async function pluginManagerHandler(req,res){
  ...
  try{
    if(action==='list'){
      ...
      const plugins=visibleRegistry.map(plugin=>{
        ...
        return {
          ...
          oauthAvailable:exposeSetup&&canUsePlugin(plugin,{isOwner})&&oauthStatus(plugin.id).liveReady,
    ...
    if(action==='providerStatus'){
      ...
      return res.status(200).json({ok:true,id,oauthStatus:status,oauthAvailable:status.liveReady,tokenFallback:status.tokenFallback,callbackUrl:publicOrigin(req)+status.callbackPath});
    }
    ...
    if(action==='startOAuth'){
      ...
      if(!canUsePlugin(plugin,{isOwner}))return res.status(403).json({error:'That plugin is not available for this account.'});
      const status=oauthStatus(id);
      if(plugin.status==='coming_soon')return res.status(409).json({error:plugin.name+' OAuth is planned but not live yet. It needs provider review, token exchange, scopes, and action safeguards before users can connect it.',oauthStatus:status,tokenFallback:status.tokenFallback});
      if(!status.configured)return res.status(501).json({error:'OAuth needs provider credentials added in Vercel before it can go live.',oauthStatus:status,tokenFallback:status.tokenFallback});
      if(!status.exchangeImplemented)return res.status(501).json({error:'OAuth for '+plugin.name+' is planned but the secure token exchange is not implemented yet.',oauthStatus:status,tokenFallback:status.tokenFallback});
