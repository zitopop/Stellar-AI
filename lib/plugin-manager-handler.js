...
const OAUTH_PLUGIN_IDS=new Set(['github']);
...
function oauthConfigured(id){
  return id==='github'&&Boolean(process.env.GITHUB_OAUTH_CLIENT_ID&&process.env.GITHUB_OAUTH_CLIENT_SECRET);
}
...
export default async function pluginManagerHandler(req,res){
  ...
  try{
    if(action==='list'){
      ...
      const plugins=PLUGIN_REGISTRY.map(plugin=>{
        ...
        return {
          ...
          oauthAvailable:OAUTH_PLUGIN_IDS.has(plugin.id)&&oauthConfigured(plugin.id),
          oauthStatus:OAUTH_PLUGIN_IDS.has(plugin.id)?oauthStatus(plugin.id):null,
          installable:availableToUser&&plugin.status!=='coming_soon',
          oauthSetupMissing:Array.isArray(plugin.setupEnv)&&plugin.setupEnv.some(name=>!process.env[name]),
          oauthSetupMissingEnv:Array.isArray(plugin.setupEnv)?plugin.setupEnv.filter(name=>!process.env[name]).slice(0,8):[],
        };
      });
      return res.status(200).json({ok:true,plugins});
    }

    if(action==='install'){    if(action==='providerStatus'){
    ...
    if(action==='startOAuth'){
      ...
      if(!canUsePlugin(plugin,{isOwner}))return res.status(403).json({error:'That plugin is not available for this account.'});
      const status=oauthStatus(id);
      if(!status.configured)return res.status(501).json({error:'OAuth is built, but this provider still needs its OAuth client ID and secret added to Vercel. Use secure token connect for now.',oauthStatus:status,tokenFallback:true});
