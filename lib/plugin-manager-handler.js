  ...
  try{
    ...
    if(action==='install'){
      ...
      if(tokenPlugin(id)&&!(await hasPluginCredential(session.email,id))){
        return res.status(200).json({ok:true,needsConnection:true,oauthAvailable:OAUTH_PLUGIN_IDS.has(id)&&oauthConfigured(id),plugin:publicPluginManifest(plugin,{enabled:false,availableToUser:true})});
      }
    ...
    if(action==='startOAuth'){
      ...
      if(!oauthConfigured(id))return res.status(501).json({error:'OAuth is ready in the app, but this provider needs its OAuth client ID and secret added to Vercel first. Use secure token connect for now.'});
