...
const INPUT_TYPES=new Set(['keyboard_shortcut','type_text','mouse_move','mouse_click']);
const APPROVAL_TYPES=new Set(['write_file','apply_patch','mkdir','run_command','open_url',...INPUT_TYPES]);
const ALL_TYPES=new Set([...SAFE_TYPES,...APPROVAL_TYPES]);
const DEFAULT_POLICY={read:true,write:true,shell:false,openUrl:true,input:false};
const PC_TASK_LIMIT=120;
...
async function setPolicy(email,input){
  const p={read:input?.read!==false,write:input?.write!==false,shell:input?.shell===true,openUrl:input?.openUrl!==false,input:input?.input===true};
...
function limitKey(email,scope,windowSeconds){const bucket=Math.floor(Date.now()/(windowSeconds*1000));return `stellar:desktop:limit:${scope}:${hash(String(email).toLowerCase())}:${bucket}`}
async function underLimit(email,scope,limit,windowSeconds){const key=limitKey(email,scope,windowSeconds);const n=Number(await redis('incr',key)||0);if(n===1)await redis('expire',key,windowSeconds+60);return n<=limit}
async function pcUsage(email){if(isOwnerEmail(email))return {unlimited:true,limit:null,used:0,remaining:null,percent:0};const used=Math.max(0,Number(await redis('get',limitKey(email,'task',3600))||0));return {unlimited:false,limit:PC_TASK_LIMIT,used,remaining:Math.max(0,PC_TASK_LIMIT-used),percent:Math.min(100,Math.round((used/PC_TASK_LIMIT)*100))}}
...
function validateArgs(type,args){
  ...
  if(type==='type_text'){const text=String(args?.text??'');if(!text||text.length>2000)throw new Error('Typed text must be between 1 and 2000 characters.');if(containsSensitive(text))throw new Error('Typing likely credentials or secrets through Stella X is blocked.')}
  if(type==='keyboard_shortcut'){const shortcut=String(args?.shortcut||'').trim();if(!shortcut||shortcut.length>40)throw new Error('Keyboard shortcut is missing or too long.')}
  if(type==='mouse_move'||type==='mouse_click'){const x=Number(args?.x),y=Number(args?.y);if(!Number.isInteger(x)||!Number.isInteger(y)||x<0||y<0||x>10000||y>10000)throw new Error('Mouse coordinates must be whole numbers between 0 and 10000.');if(type==='mouse_click'){const button=String(args?.button||'left').toLowerCase();if(!['left','right','middle'].includes(button))throw new Error('Mouse button must be left, right or middle.');const clicks=Number(args?.clicks||1);if(![1,2].includes(clicks))throw new Error('Mouse clicks must be 1 or 2.')}}
}
function cleanTask(policy,input){
  ...
  if(['write_file','apply_patch','mkdir'].includes(type)&&policy.write!==true)throw new Error('Write access is disabled.');
  if(INPUT_TYPES.has(type)&&policy.input!==true)throw new Error('Mouse and keyboard control is disabled in PC Agent permissions.');
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='status'){
      const enabled=await isPluginEnabled(session.email,'pc-agent'),usage=await pcUsage(session.email);
      const stopped=await emergencyStopped(session.email),policy=await getPolicy(session.email);
      if(!deviceId)return res.status(200).json({ok:true,paired:false,enabled,emergencyStopped:stopped,policy,usage});
      ...
      return res.status(200).json({ok:true,paired:true,enabled,deviceId,hostname:d.hostname,platform:d.platform,lastSeen:d.lastSeen,online:enabled&&!stopped&&Date.now()-Number(d.lastSeen||0)<90000,activeTask,lastResult,emergencyStopped:stopped,policy,usage});
    }
    if(!deviceId)return res.status(409).json({error:'Pair a Windows PC first.'});
    if(action==='enqueue'){
      if(!(await isPluginEnabled(session.email,'pc-agent')))return res.status(423).json({error:'PC Agent is disabled in Plugins.'});
      if(!isOwnerEmail(session.email)&&!(await underLimit(session.email,'task',PC_TASK_LIMIT,3600)))return res.status(429).json({error:'PC Agent task limit reached for this hour.'});
