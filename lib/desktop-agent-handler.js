...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='status'){
      ...
      return res.status(200).json({ok:true,paired:true,deviceId,hostname:d.hostname,platform:d.platform,lastSeen:d.lastSeen,online:!stopped&&Date.now()-Number(d.lastSeen||0)<90000,activeTask,lastResult,emergencyStopped:stopped,policy});
    }
