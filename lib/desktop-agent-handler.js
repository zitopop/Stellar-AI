...
const APPROVAL_TYPES=new Set(['write_file','apply_patch','mkdir','run_command','open_url']);
...
function validateArgs(type,args){
  ...
  if(type==='apply_patch'){
    if(typeof args.find!=='string'||!args.find||typeof args.replace!=='string')throw new Error('A patch needs exact existing text and replacement text.');
    if(args.find.length>100000||args.replace.length>100000)throw new Error('Patch is too large.');
    if(containsSensitive(args.find)||containsSensitive(args.replace))throw new Error('Patches containing likely credentials are blocked.');
  }
  if(type==='open_url'){
    const u=new URL(String(args?.url||''));if(!['https:','http:'].includes(u.protocol))throw new Error('Only http/https URLs can be opened.');
  }
}
export function cleanTask(policy,input){
