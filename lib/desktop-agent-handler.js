...
const APPROVAL_TYPES=new Set(['write_file','mkdir','run_command','keyboard_shortcut','type_text','open_url']);
...
function validateArgs(type,args){
  ...
  if(type==='type_text'){
    ...
    if(containsSensitive(text))throw new Error('Typing likely credentials or secrets through the cloud PC Agent is blocked.');
  }
  if(type==='keyboard_shortcut'){
    const shortcut=String(args?.shortcut||'').toUpperCase().replace(/\s+/g,'');
    if(!['CTRL+TAB','CTRL+SHIFT+TAB','ALT+TAB','CTRL+L','CTRL+W','CTRL+R'].includes(shortcut))throw new Error('Shortcut is not in the Stellar safe allowlist.');
  }
  if(type==='open_url'){
...
function cleanTask(policy,input){
  ...
  if(['write_file','mkdir','type_text'].includes(type)&&policy.write!==true)throw new Error('Write/typing access is disabled.');
