...
export function summarizeRecentUsage(profiles, now = Date.now()) {
  ...
  return {
    ...
    walletCredits: {
      tracked: false,
      note: 'Wallet credits are grants and top-ups; chat uses included hourly requests first and debits wallet credit only when the user has explicitly enabled eligible overage.',
    },
  };
}
