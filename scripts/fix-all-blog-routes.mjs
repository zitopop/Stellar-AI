...
for (const [slug, file] of Object.entries(routeMap)) {
  const p = path.join(root, 'blog', file);
  if (!fs.existsSync(p)) throw new Error(`Missing article file for ${slug}: blog/${file}`);
}
...
const existing = Array.isArray(cfg.rewrites) ? cfg.rewrites : [];
const nonBlogRewrites = existing.filter(r => typeof r?.source === 'string' && !r.source.startsWith('/blog/'));
const blogRewrites = Object.entries(routeMap).map(([slug, file]) => ({
  source: `/blog/${slug}`,
  destination: `/blog/${file}`
}));

cfg.rewrites = [...blogRewrites, ...nonBlogRewrites];
fs.writeFileSync(configPath, `${JSON.stringify(cfg, null, 2)}\n`);

const reparsed = JSON.parse(fs.readFileSync(configPath, 'utf8'));
const configuredBlogRoutes = reparsed.rewrites.filter(r => r.source?.startsWith('/blog/'));
if (configuredBlogRoutes.length !== 66) throw new Error(`Expected exactly 66 explicit blog rewrites, got ${configuredBlogRoutes.length}`);
if (configuredBlogRoutes.some(r => r.source === '/blog/:slug')) throw new Error('Generic /blog/:slug rewrite must not exist');
console.log('Validated and configured all 66 Stellar blog routes.');
